...
grid_rows = grid_str.split("\n")
grid = [x.split(" ") for x in grid_rows]
grid = [[int(x) for x in y] for y in grid]
grid = np.array(grid)
max_product_row = max([max([l[i] * l[i+1] * l[i+2] * l[i+3] for i in range(0, len(l)-3)]) for l in grid])
max_product_col = max([max([l[i] * l[i+1] * l[i+2] * l[i+3] for i in range(0, len(l)-3)]) for l in grid.T])
